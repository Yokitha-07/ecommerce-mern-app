...
function Countdown({ end }) {
  function calc(endTime) {
    const diff = new Date(endTime) - new Date();
    if (!endTime || Number.isNaN(diff) || diff <= 0) return "00:00:00";
...
export default function DealsSection() {
  ...
  useEffect(() => {
    fetch(`${API}/api/products?isDeal=true&limit=8`)
      .then((r) => r.json())
      .then((json) => setDeals(Array.isArray(json.data) ? json.data : []))
      .catch((err) => console.error(err));
  }, []);
  ...
  return (
    <div className="my-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {deals.map((d) => {
          // ✅ Support Task 7 (price/images at root) AND Task 8 (variants/basePrice)
          const firstVariant = d.variants?.[0];

          const price = Number(d.price ?? firstVariant?.price ?? d.basePrice ?? 0);
          const img =
            d.images?.[0] ||
